...
namespace ECM.Components
{
    ...
    public sealed class CharacterMovement : MonoBehaviour
    {
        ...
        private static readonly Collider[] _overlappingColliders = new Collider[8];
        ...
        public void Pause(bool pause, bool restoreVelocity = true)
        {
            ...
            {
                // Unpause and restore saved rigidbody state (if desired)
                ...
                else
                {
                    ...
                    _rigidbody.AddForce(zero, ForceMode.VelocityChange);
                    _rigidbody.AddTorque(zero, ForceMode.VelocityChange);
                }
        ...
        public Collider[] OverlapCapsule(Vector3 bottom, Vector3 top, float radius, out int overlapCount, LayerMask overlappingMask, QueryTriggerInteraction queryTriggerInteraction)
        {
            var colliderCount = Physics.OverlapCapsuleNonAlloc(bottom, top, radius, _overlappingColliders,
                overlappingMask, queryTriggerInteraction);

            overlapCount = colliderCount;
            for (var i = 0; i < colliderCount; i++)
            {
                var overlappedCollider = _overlappingColliders[i];
                if (overlappedCollider != null && overlappedCollider != capsuleCollider)
                    continue;

                if (i < --overlapCount)
                    _overlappingColliders[i] = _overlappingColliders[overlapCount];
            }
